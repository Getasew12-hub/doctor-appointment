...
function DoctorDashboard() {
  ...
  const {loading,DoctorDashboard,data}=doctorDashboradStore();
  ...
  const stats = [
    { label: 'Total Earnings', value:data?.total_earnings, icon: <Wallet className="text-blue-600" />, bg: 'bg-blue-50' },
    { label: 'Appointments', value: data?.totalAppointme, icon: <CalendarDays className="text-purple-600" />, bg: 'bg-purple-50' },
    { label: 'Patients', value: data?.totalPaients, icon: <Users className="text-orange-600" />, bg: 'bg-orange-50' },
  ];
    ...
    <div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          ...
          <div className="overflow-x-auto">
            {data?.getLesteAppointemnt?.length==0 && <p className='text-center my-3'>No data found</p>}
            <table className="w-full text-left">
              <tbody className="divide-y divide-gray-50">
                {data?.getLesteAppointemnt?.length>0 && data?.getLesteAppointemnt.map((app,index) => (
